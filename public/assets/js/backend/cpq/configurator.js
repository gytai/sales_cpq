define(['jquery', 'bootstrap', 'backend', 'fast'], function ($, undefined, Backend, Fast) {
    ...
    function renderGroups(schema) {
        ...
        $('#cpq-reset,#cpq-validate,#cpq-bom-run').prop('disabled', false);
        updateSummary();
        // 初始加载即做一次服务端校验，呈现默认值下的依赖/互斥与显隐状态
        runValidation();
    }
    ...
    // 服务端校验（合法性、依赖/互斥、动态显隐、BOM 均以后端返回为准）
    // validationSeq 用于测试与调试识别"最近一次校验结果"，避免读到过期面板
    var validationSeq = 0;
    function runValidation(onDone) {
        if (!currentSchema) {
            return;
        }
        ...
        Fast.api.ajax({
            ...
            $('#cpq-validation').attr('data-seq', ++validationSeq);
            if (typeof onDone === 'function') {
                onDone(validation);
            }
            return false;
        }, function (data, ret) {
            // 服务端拒绝（如公式操作数缺失）时同样落到校验面板，而不是只有一闪而过的提示
            $('#cpq-validation').empty()
                .append('<span class="label label-danger">服务端校验失败</span>')
                .append($('<p class="text-danger" style="margin-top:6px"></p>').text((ret && ret.msg) || '未知错误'))
                .attr('data-seq', ++validationSeq);
            return false;
        });
    }

    // 选择变化后防抖实时校验，给出依赖/互斥与动态显隐提示
    ...
    var Controller = {
        index: function () {
            ...
            // BOM 模拟：与 /api/cpq/v1/configurations/bom 同一口径，配置不合法时服务端拒绝
            $(document).on('click', '#cpq-bom-run', function () {
                var modelId = $('#cpq-model').val();
                Fast.api.ajax({
                    url: 'cpq/configurator/bom',
                    type: 'post',
                    data: {model_id: modelId, configuration: JSON.stringify(collectConfiguration())}
                }, function (result) {
                    renderBom(result.bom);
                    Toastr.success('BOM 已生成，配置指纹：' + (result.configuration_hash || ''));
                    return false;
                });
            });
        }
