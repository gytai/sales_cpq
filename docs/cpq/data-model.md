...
| `cpq_option_value` | 配置选项：code、group_id、material_code、default_qty、no_material、status | M1 已建 |
...
### 1.2 规则与价格表（install.sql 已建，M2 已交付管理能力）
...
| `cpq_price_book` | 价格表：code、company、scope、currency、tax_mode、version、status | M2 已交付：生命周期发布（同范围同优先级时间重叠禁止）、发布生成不可变版本 |
| `cpq_price_entry` | 价格条目：price_book_id、target_type、target_id、amount、unit | M2 已交付：父价格表可编辑守卫、导入预览 |
| `cpq_price_policy` | 三层价格策略：dimension_key、guide_price、line_floor、company_floor、cost | M2 已交付：三层关系/维度重叠校验、角色脱敏 |
| `cpq_price_rule` | 条件价格规则：condition_json、adjustment_type、adjustment_value、priority | M2 已交付：条件白名单、互斥组冲突发布校验 |
| `cpq_exchange_rate` | 汇率：source_currency、target_currency、rate、effective_at | M2 已交付 |
| `cpq_tax_rule` | 税率：country、region_id、product_type、rate、effective_at | M2 已交付（region_code 引用区域编码） |
| `cpq_fee_rule` | 费用规则：fee_type、condition_json、calculation_type、value | M2 已交付 |
| `cpq_release_version` | 发布版本：object_type、object_id、version、content_hash、effective_at、status | M2 已建，已生效不可变、未生效可撤回、回滚=重发旧内容 |

`dimension_key` 由以下维度规范化后计算哈希：公司、业务板块、国内/国际、区域、客户等级、
代理等级、指定客户、指定代理商、产品线、型号/选项、币种、单位。日期区间不进入哈希，
而是在相同维度键内单独执行生效区间重叠校验；原始维度仍单独字段存储以便查询审计。

### 1.3 客户与渠道表（install.sql 已建，M2）

| 表 | 作用 | 状态 |
| --- | --- | --- |
| `cpq_customer` | 客户：code、type、credit_code、region_id、customer_level_id、agent_id、sales_org_id、owner_id、status | M2 已建（agent_id 与 cpq_agent.customer_id 互引，不建物理外键，服务层校验） |
| `cpq_customer_level` | 客户等级：code、sort、default_discount、market_scope | M2 已建 |
| `cpq_agent` | 代理商：code、customer_id、agent_level_id、授权区域/产品线 JSON、信用额度 | M2 已建 |
| `cpq_agent_level` | 代理等级：code、sort、default_discount | M2 已建 |
| `cpq_region` | 销售区域树：code、parent_id、path、level、default_currency、sales_org_id | M2 已建，树路径由 ChannelTreeService 维护 |
| `cpq_sales_org` | 销售组织树：code、parent_id、path、level、manager_id、生效区间 | M2 已建 |
| `cpq_sales_org_member` | 组织成员：org_id、admin_id、role、生效区间 | M2 已建 |
...
| `cpq_release_version` | 发布版本（object_type、version、content_hash、effective_at） | M2 已交付（2026090303 脚本） |
