...
class ConfigurationService
{
    ...
    /**
     * BOM 缺失映射校验（GYTAI-66，方案 P20）：影响 BOM 的配置组中，
     * 每个选项必须满足以下三者之一，否则判定为缺失映射：
     *  1. 选项自身维护了 material_code（选项直接映射物料）；
     *  2. 存在以该选项为维度的已发布 BOM 映射；
     *  3. 选项被明确标记为"不产生物料"（no_material）。
     *
     * 纯函数实现，不触库；schema 结构同 ConfigurationSchemaRepository 输出。
     *
     * @param array $schema
     * @return array 缺失映射列表 [['group_code'=>,'option_code'=>,'option_id'=>,'reason'=>], ...]
     */
    public function findMissingBomMappings(array $schema)
    {
        $mappedOptionIds = [];
        foreach ($schema['bom_mappings'] ?? [] as $mapping) {
            if (!empty($mapping['option_value_id'])) {
                $mappedOptionIds[(int)$mapping['option_value_id']] = true;
            }
        }

        $missing = [];
        foreach ($schema['groups'] ?? [] as $group) {
            if (empty($group['affects_bom'])) {
                continue;
            }
            $options = $group['options'] ?? [];
            // 兼容索引化（code=>option）与列表两种形态
            foreach ($options as $option) {
                if (!empty($option['no_material'])) {
                    continue;
                }
                if (trim((string)($option['material_code'] ?? '')) !== '') {
                    continue;
                }
                if (!empty($option['id']) && isset($mappedOptionIds[(int)$option['id']])) {
                    continue;
                }
                $missing[] = [
