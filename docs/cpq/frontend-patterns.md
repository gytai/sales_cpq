...
- 工具栏：`{:build_toolbar('refresh,add,edit,del')}`，按 `fa_auth_rule` 自动显隐；
- 行内编辑/删除权限：`data-operate-edit="{:$auth->check('cpq/product_series/edit')}"`；
- 版本流按钮复用 `CpqCommon.versionButtons(baseUrl)`（提交/发布，按 `row.status` 显隐，`btn-ajax` + 中文确认文案）；直接发布用 `CpqCommon.directPublishButtons(baseUrl)`；
...
| P52 配置器 | 直接复用 §7 |
| P56/P57 报价列表与详情、P70–P73 审批 | 列表模式 + 状态徽标（§3），审批操作用 `btn-ajax` 确认按钮模式（§5） |
